      ...
      <div className="h-full border-l border-neutral-900 rounded-2xl  " >
        <div className="flex items-start px-5 flex-col text-xs gap-2 text-left  ">
          {myNfts?.selectedNft ? <div className=" w-full h-full rounded-xl">
            ...
            <Link className="border border-neutral-800 px-5 py-2 rounded-lg hover:bg-neutral-800 text-white transition-all duration-300" to={`/earn/marketplace/my-nfts/nft/?address=${myNfts?.selectedNft?.contract}&id=${myNfts?.selectedNft?.identifier}&cid=${myNfts?.selectedNft?.metadata_url}&chain=97`} >view full details</Link >
          </div>
