...
const NftListingPreview = ({myNfts,Nfts})=> {
                        ...
                        <div className="   w-1/3 flex flex-col border rounded-l-md h-[50vh] overflow-y-scroll p-2 m-0 border-neutral-800 items-start justify-start  gap-2 ">
                            {Nfts?.data && Nfts?.data?.map((nft, i) => {
                                if (nft_contract?.toLowerCase() == nft?.contract?.toLowerCase()) {
                                    return <img key={i} src={nft.image_url} className={`w-26 h-26 object-cover rounded-xl cursor-pointer ${myNfts?.selectedNft == nft && ' outline outline-pink-600 '} `} onClick={(e) => handleSelect(nft, i, e)} />
                                }
                            })}
                        </div>
                    </div>
...
export default NftListingPreview