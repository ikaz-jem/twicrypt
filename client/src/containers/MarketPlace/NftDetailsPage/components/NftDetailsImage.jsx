...
useEffect(()=>{
    ...
    const loaded = () => {
        ...
        if (!nftDetailsPageState?.metadata?.image && !imgLoad) {
            ...
            <div className="flex flex-col justify-start items-center  rounded-3xl  bg-[#000000d2] w-auto h-auto overflow-hidden border border-neutral-900 relative ">
                ...
                {<img onLoad={()=> loaded()} src={nftDetailsPageState?.imageLink} alt="Fetching image" className=" object-contain w-full relative rounded-b-md" />}
            </div>
        )
