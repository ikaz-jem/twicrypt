    ...
    <div  className={`border rounded-2xl ${!data.title && 'cursor-wait'} w-60 h-auto shadow-sm border-neutral-800 hover:shadow-lg pb-0 relative hover:translate-y-[-1%] transition-all cursor-pointer overflow-clip` }>
      <div className="flex flex-col  h-80 w-full ">
        ...
        <div className="flex justify-center items-start flex-col gap-2 mx-2 my-2 px-2 ralative  ">
          <dd className="order-first text-md font-semibold tracking-tight text-gray-200  w-full text-left truncate text-ellipsis overflow-hidden  ">
        ...
        <button disabled={data?.title || data?.identifier || data?.thumbnailUrl ? false:true} className=" rounded-md bg-blue-500 py-1 px-10 text-white hover:bg-white hover:text-black transition-all duration-200 disabled:bg-gray-400 disabled:cursor-not-allowed"> sell</button>
          </div>
        </div>
      </div>
    </div>
